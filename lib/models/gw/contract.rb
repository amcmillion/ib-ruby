...
module IB
  module BusinessDays
	...
	# Custom ErrorClass which can be fired if a verification fails
  ...
  class Contract
    ...
    def yml_file
      File.expand_path('../../../../contract_config.yml', __FILE__)
		...
		#
		#
		# The method accepts a block. The  queried contract-Object is assessable there.
    ...
    def verify!
      ...
      IB::Connection.logger.error { "multiple Contracts detected during verify!." } if c > 1
    ...
    def eod(start: nil, to: Date.today, duration: nil, what: :trades, timeout: 5)
...
#					msg.results.each { |entry| puts "  #{entry}" }
      ...
      duration = if duration.present?
                   ...
                   BusinessDays.business_days_between(start, to).to_s + " D"
                 else
                   "1 D"
                 end
      ...
      Timeout::timeout(timeout) do
        sleep 0.1
        last_time = received.pop # blocks until a message is ready on the queue
        loop do
          sleep 0.1
          break if received.empty? # finish if no more data received
        end
      end
    end
...
#																 puts "TheStrikes #{the_strikes}"
    ...
    private
		...
		#
		#
		# if :update is true, the attributes of the Contract itself are updated; otherwise the Contract is untouched
    def _verify(thread: nil, update:, &b) # :nodoc:
      ...
      if tws_request_not_nessesary
        ...
        ### send the request !
        contract_to_be_queried = query_contract #con_id.present? ? self : query_contract
        ...
        if contract_to_be_queried.present? # is nil if query_contract fails
          ...
          th = Thread.new do
            begin
              ...
              Connection.logger.error { "#{to_human} --> No ContractData received " }
            end
            ib.unsubscribe a
          end
          if thread.nil?
